...
const sdk = new CoinbaseWalletSDK({
  appName: "Saffron Helixyard (Built for Base)",
...
async function rpcHealth() {
  const c = client();
  const [reportedChainId, bn, gasPrice] = await Promise.all([
    c.getChainId(),
    c.getBlockNumber(),
    c.getGasPrice(),
  ]);
...
async function readErc20(token: string, holder: string) {
  ...
  print([
    ...
    `Token on Basescan: ${active.explorer}/address/${t}`,
  ]);
}
...
function mount() {
  ...
  h1.textContent = "Saffron Helixyard";

  const blurb = document.createElement("div");
  blurb.textContent =
    "Built for Base: connect wallet, validate chainId, and run read-only ETH + ERC-20 reads with Basescan references.";
  ...
  row.append(
    ...
    btn("RPC Health", rpcHealth),
  );

  row2.append(
    addrInput,
    btn("Read ETH Balance", () => readEthBalance((addrInput.value.trim() || session?.address || "").toString())),
    tokenInput,
    btn("Read ERC-20", () => readErc20(tokenInput.value.trim(), (addrInput.value.trim() || session?.address || "").toString())),
  );
